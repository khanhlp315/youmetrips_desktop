...
public class PlaceScreenImpl extends FXMLScreen
    ...
    public void updatePlaces(List<Place> places) {
        places = places.stream().limit(5).collect(Collectors.toList());
        _lvPlaces.setItems(FXCollections.observableArrayList(places.stream().map(place -> {
            return new PlaceCell(
                    place,
                    event -> {
                        _presenter.savePlaceDetailsId(place.getId());
